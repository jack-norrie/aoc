...
void advance_directions(std::vector<std::vector<char>> &grid,
  ...
  for (auto &direction_symbol : direction_symbols) {
    ...
    size_t r = position[0] + direction[0];
    size_t c = position[1] + direction[1];

    // Check that there is a space for theboxes to move to.
    while (check_in_bounds(grid, r, c) && grid[r][c] == 'O') {
      r += direction[0];
      c += direction[1];
    }

    if (grid[r][c] == '.') {
      // Move boxes up to first empty space
      grid[r][c] = 'O';
