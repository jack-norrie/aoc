...
size_t count_reachable_words_in_trie(const Trie &designs,
  ...
  if (designs.word.has_value()) {
    std::cout << "Reachable Design: " << designs.word.value() << std::endl;
    ++res;
  }
  ...
  for (const auto &towel : towels) {
    ...
    if (sub_designs != nullptr) {
      res += count_reachable_words_in_trie(*sub_designs, towels, seen);
    }
  }
...
int main() {
  ...
  size_t reachable_word_count =
      count_reachable_words_in_trie(designs, towels, seen);
  std::cout << "Number of Possible desings: " << reachable_word_count
            << std::endl;
